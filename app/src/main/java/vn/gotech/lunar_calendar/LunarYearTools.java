...
public class LunarYearTools {
    ...
    public static int tinhGioHoangDao(int dd, int mm, int yy) {
        // Tính số ngày tính từ điểm xuất phát (Jiazi)
        int startYear = 1900; // Năm xuất phát (theo lịch âm)
        int startGioHoangDao = 9; // Giờ hoàng đạo ứng với năm xuất phát
        int daysFromStart = tinhSoNgayTuJiazi(dd, mm, yy, startYear);
        // Tính số giờ từ nửa đêm đến thời điểm cần tính
        int hour = 12; // Giờ cần tính (tính từ nửa đêm)
        double hoursFromMidnight = hour / 2.0;

        // Tính số độ hoàng đạo tương ứng với số giờ hoàng đạo từ nửa đêm đến thời điểm cần tính
        int chuKy = 30; // Chu kỳ 12 giờ hoàng đạo
        double degreesFromStart = (hoursFromMidnight / chuKy) * 360;

        // Tính giờ hoàng đạo
        int degreeStartGioHoangDao = startGioHoangDao * chuKy;
        int degreeGioHoangDao = (int) (degreeStartGioHoangDao + degreesFromStart) % 360;
        int gioHoangDao = degreeGioHoangDao / chuKy;
        return gioHoangDao;
    }

    // Hàm tính số ngày từ điểm xuất phát (Jiazi)
    public static int tinhSoNgayTuJiazi(int dd, int mm, int yy, int startYear) {
        // Tính số năm âm lịch tính từ điểm xuất phát
        int yearsFromStart = yy - startYear;

        // Tính số ngày từ điểm xuất phát đến ngày cần tính
        int totalDays = 0;
        for (int i = startYear; i < yy; i++) {
            totalDays += tinhSoNgayTrongNam(i);
        }
        totalDays += tinhSoNgayTrongThang(mm, yy);
        totalDays += dd - 1;

        // Tính số ngày từ điểm xuất phát đến ngày cần tính
        int daysFromStart = yearsFromStart * 60 + totalDays;
        return daysFromStart;
    }

    // Hàm tính số ngày trong một năm âm lịch
    public static int tinhSoNgayTrongNam(int yy) {
        int totalDays = 0;
        for (int i = 1; i <= 12; i++) {
            totalDays += tinhSoNgayTrongThang(i, yy);
        }
        return totalDays;
    }

    // Hàm tính số ngày trong một tháng âm lịch
    public static int tinhSoNgayTrongThang(int mm, int yy) {
        int[] daysInMonth = {29, 30, 29, 30, 29, 30, 29, 29, 30, 29, 30, 29, 30}; // Số ngày trong mỗi tháng âm lịch
        boolean isLeapMonth = isLeapMonth(mm, yy); // Kiểm tra tháng này có phải tháng nhuận không
        if (isLeapMonth) {
            daysInMonth[mm - 1] = 30;
        }
    ...
    // Hàm kiểm tra tháng âm lịch có phải là tháng nhuận không
    public static boolean isLeapMonth(int mm, int yy) {
        int leapMonth = tinhThangNhuan(yy); // Tháng nhuận trong năm này
        if (leapMonth == 0) { // Năm không có tháng nhuận
            return false;
        }
        if (mm == leapMonth) { // Tháng hiện tại là tháng nhuận
            return true;
        }
        if (leapMonth > 0 && mm > leapMonth) { // Tháng hiện tại đang sau tháng nhuận
            return false;
        }
        return false;
    }

    public static int tinhThangNhuan(int yy) {
// Lấy 3 ký số phía trước của năm
        int y = yy % 1000;
// Tính tháng nhuận bằng cách lấy phần dư của phép tính y * 12.37
        return (int) (y * 12.37) % 12;
    }
}
